...
export function InterviewSection({ interviews, onChange }: InterviewSectionProps) {
    const addRound = () => {
        ...
        <div className="space-y-4 pt-4 border-t border-zinc-800">
            ...
            <div className="space-y-4">
                {interviews.map((round, index) => (
                    <div key={round.id} className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-3">
                        <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-semibold text-zinc-300">Round {index + 1}</span>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-center">
                                    <label className="text-xs text-zinc-500 font-medium uppercase tracking-wider">Questions Asked</label>
                                    <Button
                                        type="button"
                                        variant="ghost"
                                        size="sm"
                                ...
                                {round.questions.map((q, qIdx) => (
                                    <div key={qIdx} className="flex gap-2 items-center">
                                        ...
                                        <Input
                                            ...
                                            value={q}
                                            onChange={(e) => updateQuestion(round.id, qIdx, e.target.value)}
                                        />
                                    </div>
                                ))}
                            </div>
                ...
                {interviews.length === 0 && (
                    <div className="text-center py-4 bg-zinc-900/30 rounded border border-dashed border-zinc-800">
                        <p className="text-zinc-600 text-sm italic">No interviews tracked yet.</p>
                    </div>
                )}
            </div>
        </div>
